...
module Proxy::DHCP::DhcpIscRes
  class Provider < ::Proxy::DHCP::Server
    ...
    def initialize
      ...
      @config_file = Proxy::DHCP::DhcpIscRes::Plugin.settings.config
      @leases_file = Proxy::DHCP::DhcpIscRes::Plugin.settings.leases
      @reserved_file = Proxy::DHCP::DhcpIscRes::Plugin.settings.reserved
      # TODO: verify key name and secret
      @key_name = Proxy::DHCP::DhcpIscRes::Plugin.settings.key_name
      @key_secret = Proxy::DHCP::DhcpIscRes::Plugin.settings.key_secret
      @omapi_port = Proxy::DHCP::DhcpIscRes::Plugin.settings.omapi_port
    end

    def initialize_for_testing(params)
      @name = params[:name] || @name
      @service = params[:service] || service
      @config_file = params[:config_file] || @config_file
      @leases_file = params[:leases_file] || @leases_file
      @reserved_file = params[:reserved_file] || @reserved_file
    ...
    def config
      @config ||= read_config(@config_file).join("")
    end

    def leases
      @leases ||= read_config(@leases_file, true).join("")
    end

    def reserved
      @reserved ||= read_config(@reserved_file, true).join("")
    end

    def del_record subnet, record

      logger.debug "executing del_record"

      rr = parse_reserved
      if !rr.find { |item| item.mac == record.mac }.nil? && !rr.find { |item| item.ip == record.ip }.nil?
        logger.debug "Found Static Reservation: skip"
        return nil
      end
    ...
    def add_record options = {}
      ...
      logger.debug "executing add_record"

      rr = parse_reserved
      if !rr.find { |item| item.mac == options[:mac] }.nil? && !rr.find { |item| item.ip == options[:ip] }.nil?
        logger.debug "Found Static Reservation: skip"
        return nil
      end
